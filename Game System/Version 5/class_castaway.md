...
**Version 4 rebuild.** Cleaver chassis untouched — every core, major and zenith is a direct port. Minor-tier cuts: CRAB AND ROOT loses its "surgeons can't account for it" rider; SPEAKING TO STRAYS loses always-on animal direction (kin-status is colour, the once-per-session call is the mechanic); SCAVENGED ARSENAL loses no-roll crafting (the D6 improvised profile is the whole perk, Quartermaster Training grade); RUIN-WALKER's hazard-immunity replaced with a once-per-session refresh (Round the Next Corner grade) — the hazard-warning fantasy now lives only with the Chronicler, killing the cross-class duplicate. FERAL TRACKING and SALT-STILL keep their V3 conversions (mastery; Mark of Shadow grammar). Checklist unchanged: skills Tend/Hunt/Vanish/Plunder/Brave + core Endure; domains Deepwater/Ruins/Arcane + core Wilderness; matrix Blood 2/Tide 2/no Luck.

**Errata 2026-06-10:** inline tier draft replaced with a pointer to the Location Tiers system document. The Wyrd tag question is closed — the name stays, the tag now marks Tide per the glossary.

**Version 5 rebalance.** CRAB AND ROOT now treats allies (+1 step on ally treatment), giving the Castaway a share of the party-Blood healing redistributed off the Sawbone. Heal 3→4; nothing else changed.
