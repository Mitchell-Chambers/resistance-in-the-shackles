...
**Version 4 rebuild.** Already the cleanest class (Witch chassis, near-verbatim) — light surgery only. DEEPSPEAK's free animal-interrogation gated to one question per session; THE DROWNED PLACES' always-know converted to a once-per-session ask (Lost It All grammar) tied to the dive; TIDEBLOOD's immersion heal gated per session. THE PACT keeps the V3 D4/D6 economy; HEX SIGHT keeps the V3 split (free presence-sense as colour, rolled identification). All six majors and all three zeniths are untouched ports. Checklist unchanged: skills Vanish/Hunt/Reckon/Endure/Tend + core Command; domains Faith/Ruins/Deepwater + core Arcane; matrix Tide 2/Reputation 2/no Gold.

**Version 5 rebalance.** WITCH'S HANDS gains a direct D4 ally heal alongside its draughts. Heal 2→3; absorbs party healing redistributed off the Sawbone.

**Version 5 (count trim).** THE PACT-BEAST major cut to bring the class to the 23-ability baseline (5 majors). Its bound-creature fantasy is already carried by the FAMILIAR minor; deliberately NOT folded in, to avoid over-empowering the minor. Combat untouched (Sea-Change + Brine Calls to Brine + Great and Terrible hold the floor).
