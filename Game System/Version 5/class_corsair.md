...
**Version 4 rebuild.** Hound chassis, now more faithfully. The big change: **DREAD NAME cut entirely** — its Staredown conversion duplicated the Errant's DUELLIST'S GAZE across classes. Replaced with **THE BLACK SPOT**, a direct port of the Hound's CONDEMN with all three sub-minors (Judge/Jury/Executioner), which the set had never used and which is the most pirate-shaped ability in Heart. Other cuts: IRON STOMACH loses its second perk (Major→Minor downgrade); KILLING GROUND and RAIDER'S COVES converted from always-know to once-per-session ask grammar; WEAPON OF CHOICE (permanent unconditional Brutal — above minor grade) replaced with **NOTCHES ON THE HILT**, a direct port of the Hound's KILL COUNT. AFTERMATH now rolls. Cores, remaining majors (Unstoppable/Deck-Fighter/Shipshape — all Hound ports) and zeniths otherwise unchanged, with one deliberate divergence: **SCARS LIKE MEDALS** no longer ports the Hound's "+1 per ongoing fallout" (uncapped, fired off any track). It now reads "+1 while you have ongoing Blood or Reputation fallout; +2 while you have both" — hard-capped at +2 and keyed to the class's two matrix pillars (Blood 2 / Reputation 2), which also resolves the name (scars = Blood, medals = Reputation). Checklist unchanged: skills Command/Endure/Plunder/Hunt/Tend + core Brave; domains Port/Shallows/Outlaw + core Deepwater; matrix Blood 2/Reputation 2/no Gold.

**Errata 2026-06-10:** exchange durations rewritten to fiction-scoped phrasing (Boarding Master, Boarding Action), per the system documents.

**Version 5 rebalance.** OLD SALT'S STITCHES extended to patch the crew (D6 ally Blood, or downgrade a Minor Blood fallout, once per situation). Heal 2→3; absorbs party healing redistributed off the Sawbone.
