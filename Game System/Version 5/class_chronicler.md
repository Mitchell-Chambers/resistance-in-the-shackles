...
**Version 4 rebuild.** Light surgery — the Junk Mage chassis was already sound. RUIN SOVEREIGN is the one real change: its always-on peer-status and hazard-immunity now sit behind a Reckon+Ruins roll on entry (announce yourself in the old forms), making it a cast spell like the rest of the suite. PROVENANCE trimmed to a single mastery clause (the always-know-a-buyer rider cut — NOSE-grade info belongs to the Sharper); EXCAVATOR'S EYE and THE ANNOTATED CHART keep ask grammar, once per session; OBJECT READING and ACADEMIC NETWORK keep their V3 gates; TONGUE OF THE DEAD keeps its knowledge-colour (reading a language you know is not a stake). Untouched: both cores (Archive's two facts are bounded by first-encounter; WHERE THE PAST LINGERS is Ravening Knowledge), GRAVE GOODS (Sacrifice), all tradition-suite majors, SPELL BEYOND MEMORY (V3 pricing), all zeniths. Checklist unchanged: skills Plunder/Tend/Deceive/Navigate + core Reckon; domains Colonial/Shallows/Faith/Arcane + core Ruins (5/5); matrix Tide 2/Gold 2/no Luck. Forbidden: Brave, Command.

**Version 5 rebalance.** Combat raised to the protected floor and the Knowledge spike trimmed (+9→~+5): new THE EXCAVATOR'S ANSWER (Plunder; Piercing vs stone/bone/construct/undead); THE LIGHT OF DROWNED AZLANT converted to a relic-weapon (Hunt D8, reveal kept as a sub). OBJECT READING folded into THE ARCHIVE core (Tend preserved); EXCAVATOR'S EYE + THE ANNOTATED CHART merged into THE DIG MAPPED. Skill/domain footprint unchanged.
