...
**Version 4 rebuild.** The omen-stack is broken up — the class had five free-information abilities; it now has two (STAR-READER, the gated CHART core) plus the church's sub. Cuts and replacements: READ THE OMEN replaced by **LIAR'S BURDEN**, a direct port of the Heretic's minor (D4 stress to liars, rolled — and exactly this god's business); WEATHERWISE rebuilt from free forecasting into a defensive perk (first weather hazard one dice size smaller — Iron Stomach grammar); DEAD RECKONING trimmed to one perk; LAST RITES rebuilt around the Deadwalker's LAST RITES (one question of the dead). THE PERFECT COURSE no longer deletes journeys: it is now a D8 boon plus mastery against the worst peril, with Pursuit/Escape converted to mastery/dice-step grammar. Kept untouched: MINISTRATIONS (Heretic core), INTERCESSION (Broker), HUNGRY CHURCH (Network), NAME SPOKEN OVER WATER (Oath of Sagacity), STORM CALLING (the set's model big active), all zeniths. Checklist unchanged: skills Tend/Reckon/Command/Carouse + core Navigate; domains Outlaw/Port/Arcane/Deepwater + core Faith (5/5); matrix Luck 2/Tide 2/no Blood. Forbidden: Deceive, Vanish.

**Errata 2026-06-10:** Storm Calling and The Eye rewritten to pure-Heart action-result pacing; equipment conformance — Tide is not equipment-healable, so the bandages are Tend Blood only (the soul-care lives in MINISTRATIONS and at haunts, where it belongs).

**Errata 2026-06-10 (second pass):** named-deity guidance added as a character-creation sidebar (Pathfinder deities, Besmara foremost) — resolves the pending item carried since Pass 1's Divine Sanction was cut.

**Version 5 rebalance.** Combat raised to the protected floor: STAR-READER replaced by BLESSED STEEL (Brave; Brutal vs oathbreakers/desecrators), its sky-omen reading merged into CONFESSOR; STORM CALLING now counts as combat. Footprint shifts 5/5 → **6 skills** (gained Brave) / **4 domains** (Arcane now via the PILGRIM ROUTES pick).
